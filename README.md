...
- **Task editing** — edit title, description, and priority via a click-to-open dialog
- **File attachments** — attach files to any task; stored natively in ArcGIS
- **OAuth2 login** — signs in via your ArcGIS Online account (implicit grant flow)
...
Open [auth.js](auth.js) and update the `appId` constant with your Client ID:
...
4. The app provisions a Feature Service in your ArcGIS Online content and redirects you to the new board

### Loading an existing board

- Use the **Load Board** dropdown in the nav bar — it lists all Feature Services tagged `kanban` in your ArcGIS Online account
...
You can set a custom task ID prefix by adding `prefix=XYZ` to the layer's **Acknowledgments** field in ArcGIS Online. For example, setting `prefix=ABC` will produce task IDs like `ABC-T1`, `ABC-T2`, etc.
...
├── create.html     — New board / board selection page
...
├── mvp.css         — Base CSS framework (MVP.css)
